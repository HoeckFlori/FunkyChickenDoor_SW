...
{
  public:
    WidgetBase(IModel *model, Adafruit_GFX *tft, uint16_t colorBackground, uint16_t colorFrames, uint16_t colorText, int16_t x0, int16_t y0);
    ~WidgetBase() = default;
  ...
  protected:
    ...
    const int16_t m_y0; // initial position top (from ctor)
